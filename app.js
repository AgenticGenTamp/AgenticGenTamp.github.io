...
function selectEnvironment(id, {autoplay=false}={}){
 ...
 $('#env-bars').innerHTML=order.map(id=>{const m=data.methods.find(x=>x.id===id),r=env.results[id];const name={claude:'Claude Code with Opus 5',codex:'Codex with GPT-5.6 Sol',astra:'Codex with GPT-6 Astra',genplan:'LLMGenPlan',oneshot:'One-shot',planner:'Planner',source:'Claude Code with Opus 5 + source',astraSource:'Codex with GPT-6 Astra + source'}[id];return `<div class="env-row"><strong>${name}</strong><span class="score-track"><span class="score-fill" style="--w:${r?r.mean*100:0}%;--c:${m.color}"></span></span><span class="env-value">${r?`${whole(r.mean)} <small>[${whole(r.min)}–${whole(r.max)}]</small>`:'Not available'}</span></div>`;}).join('');
}
...
function renderSourceComparison(){
 ...
 const pairs=[['Claude Code with Opus 5','claude','source'],['Codex with GPT-6 Astra','astra','astraSource']];
...
loadJSON('data/benchmark.json?v=naming-1').then(result=>{data=result;renderRanking();renderEnvironmentList();renderSourceComparison();}).catch(error=>{console.error(error);$('#scope-note').textContent='The interactive results could not load. Please download the CSV or read Tables I–II in the paper.';});
