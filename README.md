...
- Source-access references: Claude Code / Opus 5 + source and Codex / GPT-6 Astra + source are always shown separately because their interface differs from the main setting.
- Min/max in the explorer are across five runs, not confidence intervals.
- Table III computation times compare Claude Code / Opus 5, Claude Code / Opus 5 + source, Codex / GPT-6 Astra, and Codex / GPT-6 Astra + source on five shared environments: BalanceBeam 3D, Dynamo 3D, PDDLStream Packing, PDDLStream Rovers, and Tossing 3D. Per run, the time is the median over episodes of 1000 × policy time / steps; each method averages its own runs with 100% held-out success per environment; values are equal-environment means with min/max across environment-level means. These are not all-environment averages or matched synthesis-seed comparisons. The Astra + source row (16.644 ms, 0.053–45.602) is computed from its archived runs with this rule and recorded under `computationTime` in `data/astra-source-results.json`. Tables I–II of the supplied PDF still report Codex / GPT-5.6 Sol; Astra’s added success rates on the website come from the separately documented archives, not those tables.
- The supplied PDF is kept byte-for-byte unchanged. Its Table III and caption were updated, while the nearby body text still describes the previous 44-pair timing analysis. The website uses the new table and caption: 10.949, 86.433, and 1.823 ms/action, plus 16.644 ms/action for Astra + source.
